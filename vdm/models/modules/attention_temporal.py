...
from vdm.models.modules.util import (
    GEGLU,
    exists,
    default,
    Normalize,
...
from vdm.models.graphTransformer.graph_encoder import RecurrentGraphTransformer
...
class BasicTransformerBlockST(nn.Module):
    ...
    def __init__(self,
        ...
        if XFORMERS_IS_AVAILBLE:
            ...
            self.attn2 = CrossAttention(query_dim=dim, context_dim=context_dim,
                                        heads=n_heads, dim_head=d_head, dropout=dropout, **kwargs, )
            self.attn3 = CrossAttention(query_dim=dim, context_dim=dsg_dim,
                                        heads=n_heads, dim_head=d_head, dropout=dropout, **kwargs, )

            RGTrm_params = dict()
            RGTrm_params['L'] = n_layers
            RGTrm_params['out_dim'] = dim
            RGTrm_params['residual'] = True
            RGTrm_params['edge_feat'] = True
            RGTrm_params['n_heads'] = n_heads
            RGTrm_params['in_feat_dropout'] = dropout
            RGTrm_params['dropout'] = dropout
            RGTrm_params['layer_norm'] = True
            RGTrm_params['batch_norm'] = True
            RGTrm_params['self_loop'] = True
            RGTrm_params['lap_pos_enc'] = True
            RGTrm_params['pos_enc_dim'] = dim
            RGTrm_params['wl_pos_enc'] = True
            RGTrm_params['in_dim'] = dim

            self.gtrans = RecurrentGraphTransformer(RGTrm_params)
        ...
        self.attn3_tmp = TemporalCrossAttention(query_dim=dim, heads=n_heads, dim_head=d_head, dropout=dropout,
                                                # cross attn
                                                context_dim=None,
                                                # temporal attn
                                                temporal_length=temporal_length,
    ...
    def _forward(self, x, context=None, dsg=None, mask=None, ):
        ...
        # spatial cross attention for dsg
        x = rearrange(x, 'b c t h w -> (b t) (h w) c')
        if dsg is not None:
            dsg_ = []
            for i in range(len(dsg)):
                G_ = []
                G_k = None
                for k in range(dsg[i].shape[0]):
                    node = dsg[i][k].ndata['feat'].to(x.device)  # num x feat
                    edge = dsg[i][k].edata['feat'].to(x.device)  # num x feat
                    try:
                        lap_pos_enc = dsg[i][k].ndata['lap_pos_enc'].to(x.device)
                        sign_flip = torch.rand(lap_pos_enc.size(1)).to(x.device)
                        ...
                        lap_pos_enc = None
                    try:
                        wl_pos_enc = dsg[i][k].ndata['wl_pos_enc'].to(x.device)
                    except:
                        wl_pos_enc = None
                    G_k = self.gtrans(dsg[i][k], node, edge, G_k, lap_pos_enc, wl_pos_enc)
                    G_.append(G_k)
                G_ = torch.cat(G_, dim=0)
                dsg_.append(G_.unsqueeze(0).repeat(t, 1, 1))
            dsg_ = torch.cat(dsg_, dim=0)
        else:
            dsg_ = None
        x = self.attn3(self.norm2(x), context=dsg_) + x
        x = rearrange(x, '(b t) (h w) c -> b c t h w', b=b, h=h)

        # temporal cross attention for dsg
        x = rearrange(x, 'b c t h w -> (b h w) t c')
        x = self.attn3_tmp(self.norm5(x), context=None, mask=mask) + x
...
class STAttentionBlock(nn.Module):
    def __init__(
        ...
        if use_relative_position:
            self.relative_position_k = RelativePosition(num_units=channels // self.num_heads,
                                                        max_relative_position=temporal_length)
            self.relative_position_v = RelativePosition(num_units=channels // self.num_heads,
                                                        max_relative_position=temporal_length)

        self.proj_out_s = zero_module(
            conv_nd(1, channels, channels, 1))  # conv_dim, in_channels, out_channels, kernel_size
