...
from vdm.models.modules.attention_temporal import STAttentionBlock
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, dsg=None, **kwargs):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb, **kwargs)
            elif isinstance(layer, STTransformerClass):
                x = layer(x, context, dsg, **kwargs)
            else:
                x = layer(x)
        return x
...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None,
        ...
        self.dims = dims
        if use_conv:
            self.conv = conv_nd(dims, self.channels, self.out_channels, (kernel_size_t, 3, 3),
...
class UNetModel(nn.Module):
                ...
                self.input_blocks.append(
                    TimestepEmbedSequential(
                        ...
                        if resblock_updown
                            ...
                            ch, conv_resample, dims=dims, out_channels=out_ch, kernel_size_t=kernel_size_t,
                            padding_t=padding_t
                        )
                    )
                )
            ...
            for i in range(num_res_blocks + 1):
                ...
                if level and i == num_res_blocks:
                    ...
                    layers.append(
                        ...
                        if resblock_updown
                        else Upsample(ch, conv_resample, dims=dims, out_channels=out_ch, kernel_size_t=kernel_size_t,
                                      padding_t=padding_t)
                    )
                    ds //= 2
    ...
    def forward(self, x, timesteps=None, time_emb_replace=None, context=None, y=None, dsg=None, **kwargs):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        :param context: conditioning plugged in via crossattn
        :param y: an [N] Tensor of labels, idf class-conditional.
        :param dsg: an [M] SG, prompt plugged in vai crossattn.
        :return: an [N x C x ...] Tensor of outputs.
        """
...
class FrameInterpPredUNet(UNetModel):
    ...
    def forward(self, x, timesteps, context=None, y=None, s=None, mask=None, **kwargs):
        if s is not None:
            ...
            mask_ = mask[:, :, :, 0, 0]  # b1t
            t = mask.shape[2]
            emb_mix = emb.unsqueeze(2).repeat(1, 1, t) * (1 - mask_) + s_emb.unsqueeze(2).repeat(1, 1,
                                                                                                 t) * mask_  # bc becomes bct
