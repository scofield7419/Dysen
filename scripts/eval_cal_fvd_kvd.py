...
from vdm.utils.common_utils import instantiate_from_config, shift_dim
from scripts.fvd_utils.fvd_utils import get_fvd_logits, frechet_distance, load_fvd_model, polynomial_mmd


def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--predict_path', type=str, default='predict data path')
    parser.add_argument('--gold_path', type=str, default='gold data dir')
...
def run(args, run_id, fake_path, i3d, loader, device):
    ...
    for i in tqdm(range(n_batch), desc="Extract Fake Embedding"):
        fake_embeddings.append(
            get_fvd_logits(fake_data[i * args.batch_size:(i + 1) * args.batch_size], i3d=i3d, device=device,
